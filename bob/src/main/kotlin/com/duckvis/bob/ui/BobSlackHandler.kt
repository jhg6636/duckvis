...
import com.duckvis.bob.dtos.SlackRequestDto
import com.duckvis.bob.dtos.TicketDto
import com.duckvis.bob.services.BobService
...
import com.duckvis.bob.exceptions.*
...
//        println(text)
//        println(userCode)
        val userId: Long = userRepository.findByCodeAndPath(userCode, pathType)?.id
        ...
        try {
            when (bobCommand) {
                ...
                is ResponseMeSpecificPlace -> {
                    val mealType = BobTimeType.of(TimeHandler.nowDateTime())
                    return TicketDto(
                        bobService.responseMeSpecificPlace(userId, bobCommand.city),
                        ticketRepository.isFirst(mealType, BobStyleType.ANYTHING)
                    ).toString()
                }
                is ResponseVegetarian -> {
                    val mealType = BobTimeType.of(TimeHandler.nowDateTime())
                    return TicketDto(
                        bobService.responseVegetarian(userId),
                ...
                is ResponseVegetarianSpecificPlace -> {
                    val mealType = BobTimeType.of(TimeHandler.nowDateTime())
                    return TicketDto(
                        bobService.responseVegetarianSpecificPlace(userId, bobCommand.city),
                        ticketRepository.isFirst(mealType, BobStyleType.VEGETARIAN)
                    ).toString()
                }
                ...
                is ResponseChangeLivingCity -> {
                    return try {
                        val changedCityName =
                            bobService.responseChangeLivingPlace(userId, bobCommand.city).city.cityName
