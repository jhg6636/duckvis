...
class TimeHandlerTest constructor() {
    ...
    fun `12월 31일 22시 58분에 이 달의 마지막 날을 확인한다`() {
        ...
        // when
        val lastDayOfMonth = TimeHandler.monthEndTime(lastDay)
    ...
    fun `12월 31일 20시 58분에 이 달의 첫 날을 확인한다`() {
        ...
        // when
        val lastDayOfMonth = TimeHandler.monthStartTime(lastDay)
    ...
    fun `1월 2일에 이 주의 첫 날을 확인한다`() {
        ...
        // when
        val firstDayOfWeek = TimeHandler.weekStartTime(day)
    ...
    fun `일요일 21시 이후에 이 주의 마지막 날을 확인한다`() {
        ...
        // when
        val lastDayOfWeek = TimeHandler.weekEndTime(day)
    ...
    fun `오늘 23시 59분에 오늘의 시작 시간을 확인한다`() {
        // given
        val now = TimeHandler.nowDateTime().withHour(23).withMinute(59)

        // when
        val startTime = TimeHandler.dayStartTime(now)
    ...
    fun `1월 1일 9시 00분에 오늘의 시작 시간을 확인한다`() {
        ...
        // when
        val startTime = TimeHandler.dayStartTime(day)
    ...
    fun `오늘 20시 33분에 오늘의 끝 시간을 확인한다`() {
        // given
        val now = TimeHandler.nowDateTime().withHour(20).withMinute(33)

        // when
        val endTime = TimeHandler.dayEndTime(now)
