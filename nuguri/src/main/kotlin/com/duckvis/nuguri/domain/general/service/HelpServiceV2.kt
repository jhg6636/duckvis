...
class HelpServiceV2(
  ...
  override val type: CommandMinorType
    get() = CommandMinorType.HELP

  override fun response(serviceRequestDto: NuguriServiceRequestParameterDto): String {
    val params = serviceRequestDto as NuguriHelpRequestParameterDto
    val user = userRepository.findByName(params.userName) ?: throw NuguriException(ExceptionType.NO_SUCH_USER)
    val userTeams = userTeamRepository.findAllByUserId(user.id)

    val helpOption = params.helpOption ?: HelpOption.HELP_OPTIONS

    return when {
      user.isAdmin -> adminHelpMessage(helpOption)
      userTeams.any { userTeam -> userTeam.isManager } -> managerHelpMessage(helpOption)
      !user.isBot && !user.isGone -> normalHelpMessage(helpOption)
